...
public abstract class WaveTable
{
	 ...
	 * @param phase_offset offset in samples
	 * @param duty
	 * @param vol
	 * @param wme
	 * @param lfo
	 ...
	 * @param note_on
	 */
	public void SynthSamples(float[] oStream, float pFreq, float phase_offset,
		float duty, float vol, WaveModulation wme, float lfo[], float modarg,
		int samplecount, boolean note_on)
	{
		...
		for(int i = 0; i < samplecount; i++)
		{
			...
			{
				if(duty == 0.0)	//we shouldn't be here
				{
			...
			{
				if(duty == 1.0)	// we shouldn't be here
				{
